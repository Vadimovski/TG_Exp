...
class TelegramExporter:
    ...
    async def export_chat(self, chat_id, file_path):
        messages = []
        max_id = 0
        async for message in self.client.iter_messages(chat_id):
            ...
            messages.append(f"[{date_str}] (ID {sender}): {text}\n")
        # Записываем так, чтобы новые сообщения оказались в начале файла
        with open(file_path, "w", encoding="utf-8") as f:
            for line in messages:
                f.write(line)
        return max_id

    async def update_chat(self, chat_id, file_path, last_message_id):
        ...
        async for message in self.client.iter_messages(chat_id, min_id=last_message_id):
            ...
            sender = message.sender_id if message.sender_id else "UnknownSender"
            text = message.message if message.message else ""
            new_messages.append(f"[{date_str}] (ID {sender}): {text}\n")
        if new_messages:
            old_content = ""
...
class TelegramExporterUI:
    def __init__(self, root):
        ...
        # Словарь для обновляемых чатов.
        # Ключ: (block_name, chat_name, chat_id)
        # Значение: {"last_id": last_message_id, "file_path": file_path}
        self.updates = {}
    ...
    def setup_main_frame(self):
        ...
        self.main_frame.columnconfigure(1, weight=1, minsize=500)
        ...
        # Правая часть: блоки экспорта и общий таймер
        self.right_frame = tk.Frame(self.main_frame, bg="white")
        self.right_frame.grid(row=0, column=1, sticky="nsew", padx=5, pady=5)
        # Верхняя панель для кнопки "Добавить блок" и общего таймера
    ...
    def show_main_frame(self):
        ...
        # После подключения можно запустить глобальный таймер
    ...
    def add_block(self, initial_data=None):
        block = ChatBlock(self.blocks_container, save_callback=self.save_blocks, export_callback=self.export_single_chat)
        if initial_data:
    ...
    def export_single_chat(self, chat_value, block_name, chat_entry_obj):
        ...
        export_dir = os.path.join("tg_exp", block_name)
        os.makedirs(export_dir, exist_ok=True)
        file_path = os.path.join(export_dir, f"{chat_name}.txt")
        ...
        def run_export():
            future = asyncio.run_coroutine_threadsafe(
                self.exporter.export_chat(chat_id, file_path),
                self.exporter_loop
            )
            try:
                last_id = future.result()  # Получаем последний id экспортированных сообщений
                self.updates[key] = {"last_id": last_id, "file_path": file_path}
    ...
    def start_global_timer(self, seconds):
        self.global_timer_seconds = seconds
        self.global_countdown(self.global_timer_seconds)
...
class ChatBlock:
    def __init__(self, parent, save_callback, export_callback):
    ...
    def add_chat(self, initial_value=""):
        ...
        # Создаём объект для хранения информации о чате
